...
$connect = new PDO("mysql:host=localhost;dbname=chatapp;charset=utf8mb4", "root", "");
...
{
	$query = "SELECT username FROM pengguna WHERE user_id = '$user_id'";
	$statement = $connect->prepare($query);
	$statement->execute();
	$result = $statement->fetchAll();
	foreach($result as $row)
...
{
	$query = "
	SELECT is_type FROM detail_login 
	WHERE user_id = '".$user_id."' 
	ORDER BY last_activity DESC 
	LIMIT 1
	";	
